...
\begin{figure}[htb]
	...
	\legend{Fonte: \url{https://www.edenai.co/post/top-10-object-detection-apis}}
	\end{center}	
\end{figure}
...
\begin{figure}[htb]
	...
	\legend{Fonte: \url{https://www.cerebras.ai/blog/cerebras-inference-3x-faster}}
	\end{center}	
\end{figure}

Os limites de uso gratuito dependem do modelo a ser escolhido. 
No caso do modelo da Meta \texttt{llama-3.3-70b}, por exemplo, os limites são listados no \autoref{quad:cerebra_limites}:

\begin{quadro}[htb]
    \centering
    \caption{Limites de Uso da Cerebra}
    \label{quad:cerebra_limites}
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{Categoria} & \textbf{Por minuto} & \textbf{Por hora} & \textbf{Por dia} \\
        \hline
        \textbf{Solicitações} & 30 & 900 & 14400 \\
        \hline
        \textbf{Uso de Tokens} & 64 mil & 1 milhão & 1 milhão \\
        \hline
    \end{tabular}
\end{quadro}
...
Considerando os limites do modelo \texttt{llama-3.3-70b-versatile} apresentados no \autoref{quad:groq_limites},
percebe-se que os valores são consideravelmente mais restritivos que os da Cerebra. 

\begin{quadro}[htb]
    \centering
    \caption{Limites de Uso do Groq}
    \label{quad:groq_limites}
    \begin{tabular}{|l|c|c|}
        \hline
        \textbf{Categoria} & \textbf{Por minuto} & \textbf{Por dia} \\
        \hline
        \textbf{Solicitações} & 30 & 1000 \\
        \hline
        \textbf{Uso de Tokens} & 12 mil & 100 mil \\
        \hline
    \end{tabular}
\end{quadro}
...
\begin{quadro}[htb]
    ...
    \label{quad:gemini_limites}
    \begin{tabular}{|l|c|c|}
        \hline
        \textbf{Categoria} & \textbf{Por minuto} & \textbf{Por dia} \\
        \hline
        \textbf{Solicitações} & 10 & 250 \\
        \hline
        \textbf{Uso de Tokens} & 250 mil & - \\
        \hline
    \end{tabular}
\end{quadro}
...
Cerebras possui opções bem abrangentes do mundo das LLMs de código aberto e, por isso, 
também foi escolhida para teste. 
Já  a Groq possui algumas opções a mais que a Cerebra, mas nenhuma que justifique o seu uso, 
considerando que suas restrições são bem menos generosas.

Portanto, os testes utilizaram algumas LLMs disponibilizadas pela Cerebra e o modelo da Google.
