...
Os navegadores normalmente permitem uma variedade de extensões, incluindo modificações na interface do usuário, gerenciamento de \textit{cookies}, bloqueio de anúncios e scripts e estilos personalizados de páginas da \textit{web}.
...
substitui páginas de fundo por ``\textit{Service Workers}'' com recursos limitados para reduzir o uso de recursos e proíbe código hospedado remotamente. A arquitetura completa de uma extensão genérica feita por Manifest V3 pode ser vista na \autoref{fig:extensions_arquitetura}.

\begin{figure}[htb]
	\begin{center}
	\caption{Arquitetura completa de uma extensão genérica}
...
Para essa extensão, os componentes mais importantes são o ``\textit{Content Script}'' (\texttt{contentScript.js}) e o ``\textit{Background Service Worker}'' (\texttt{background.js}).
O \textit{content script} é a parte da extensão que consegue conversar diretamente com o DOM (\textit{Document Object Model}) da página que o navegador está acessando,
que é basicamente a estrutura em si do site, ou seja, tudo o que o usuário vê.
O \textit{content script} não é capaz de receber as permissões mais elevadas requisitadas no manifesto, 
para isso, ele precisa enviar uma mensagem para o \textit{background service worker}, que possui esse privilégio e o retorna com a informação solicitada.
Toda essa aparente complicação a mais garante a segurança da extensão \cite{extensionvulnerabilities2010}. 
A arquitetura com os componentes utilizados neste projeto específico está ilustrada na \autoref{fig:extensions_arquitetura_especifica}. 

\begin{figure}[htb]
	\begin{center}
	\caption{Arquitetura utilizada no projeto}
    \label{fig:extensions_arquitetura_especifica}
	\includegraphics[width=0.8\textwidth]{USPSC-img/extensions-architecture_especifica.png} \\
	\fonte{\url{https://youtu.be/TRwYaZPJ0h8?si=2gh3gnVSlcj5LRXF&t=194} e editado pelo autor}
	\end{center}
\end{figure}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Vale destacar que um estudo mais robusto envolveria um número maior de imagens. A limitação de requisições por dia limitou essa etapa e os resultados atingidos se mostraram satisfatórios, como será visto no \autoref{chap:testes_resultados}.
	\end{itemize}
...
\begin{citacao}
Os LLMs geram texto prevendo a próxima palavra (ou melhor, o próximo token) de acordo com uma distribuição de probabilidade 
\textelp{}.
O parâmetro temperatura do LLM modifica essa distribuição. Uma temperatura mais baixa basicamente torna os \textit{tokens} com a maior probabilidade de serem selecionados; uma temperatura mais alta aumenta a probabilidade de um modelo selecionar \textit{tokens} menos prováveis. Isso acontece porque um valor de temperatura mais alto introduz mais variabilidade na seleção de \textit{tokens} do LLM. Diferentes configurações de temperatura introduzem diferentes níveis de aleatoriedade quando um modelo de IA generativa produz texto.\footnote{O que é temperatura do LLM? - IBM: \url{https://www.ibm.com/br-pt/think/topics/llm-temperature}}
\end{citacao}
...
Em um primeiro momento, a extensão proposta apenas analisa as postagens do Instagram dispostas no ``\textit{feed}'', 
e somente as imagens. Não são avaliados os ``\textit{stories}'' ou vídeos, porém, como o foco aqui é a prova de conceito, 
abranger esses casos seria uma adaptação de código, pois se o algoritmo funciona bem para a as imagens do feed, 
é natural também funcionar para os \textit{stories} ou vídeos. 
Além disso, o código foi estruturado deixando aberturas para ser adaptado à outras abas do Instagram (\textit{reels}, \textit{stories}, \textit{explore}), 
...
\textit{content script}, \textit{background service worker}, \textit{frontend}, e os \textit{scripts} de teste utilizados para validar o funcionamento do \textit{backend}.

Por conta das limitações de privilégio explicada na \autoref{sec:manifest_v3}, o \textit{backend} é dividido entre o \textit{content script} e o \textit{background service worker}. 
Já o \textit{frontend} tem seus estilos definidos em um arquivo CSS (\autoref{cap:apendice_css}) e é injetado através do \textit{content script}, que insere código HTML na página do Instagram.
...
A extensão possui dois fluxos principais de execução: o primeiro engatilhado quando a página atualiza (ele checa as postagens iniciais da página), e o segundo engatilhado quando novas postagens são carregadas dinamicamente na página (\textit{infinite scroll}).
Ambas as situações são ilustradas nas Figuras \ref{fig:fluxo_atualizacao} e \ref{fig:fluxo_mutacao}, respectivamente, e serão explicadas na atual seção.

\begin{figure}[htb]
	\begin{center}
	\caption{Fluxo a partir da atualização da página}
    \label{fig:fluxo_atualizacao}
	\includegraphics[width=0.8\textwidth]{USPSC-img/fluxo_pagina_atualiza.png} \\
...
\begin{figure}[htb]
	\begin{center}
	\caption{Fluxo a partir da mutação da página}
    \label{fig:fluxo_mutacao}
	\includegraphics[width=0.8\textwidth]{USPSC-img/fluxo_pagina_mutacao.png} \\
	\fonte{Elaborado pelo autor}
	\end{center}
\end{figure}
...
Outro papel do \textit{background} é receber as URLs das imagens que o \textit{content script} identificou na página e fazer a chamada para a API de geração de texto, retornando a decisão de censura para o \textit{content script}.
O recebimento das URLs é feito pelo ouvinte de mensagens (\autoref{lst:background_onMessage}).

\begin{lstlisting}[ 
		label={lst:background_onMessage}, 
		caption={Ouvinte de mensagens no \textit{background}}
	]
...
A chamada à API é feita pelo \textit{background} pois se a chave de API ficasse no \textit{content script}, ela ficaria visível em texto puro para qualquer pessoa que inspecionasse o código da página, assim, a recomendação importar a chave no \textit{background}, onde ela fica em um processo separado e isolado, longe dos olhos do usuário comum.
...
faz uma lógica simples a partir da resposta para decidir se a imagem deve ser censurada ou não, retornando 0 (não censurar), 1 (censurar) ou 2 (erro) para o \textit{content script}.
...
Assim que o \textit{background} avisa que a página atualizou, o \textit{content script} ouve a mensagem (\autoref{lst:content_onMessage}) e inicia a verificação das postagens presentes na página.
...
A função \texttt{processImage()} (\autoref{lst:processImage}) envia a URL da imagem para a função responsável por chamar o \textit{background} (\texttt{checkIfAdultization()}), que faz a análise e retorna a decisão de censura. 
Ela também é responsável por chamar as funções que montam o \textit{frontend}, 
...
\begin{lstlisting}[
  ...
  caption={Função resumida que envia a imagem para análise e monta o \textit{frontend}}
]
...
A função \texttt{checkIfAdultization()} envia a mensagem para o \textit{background}, que a ouve (\autoref{lst:background_onMessage}), faz a análise e retorna a decisão de censura, como já explicado na \autoref{sec:background}.

Além de checar as postagens dispostas inicialmente, o \textit{content script} também observa mudanças no DOM da página, percebendo quando novos nós são carregados ao rolar a página para baixo.
...
\subsection{\textit{Frontend}} \label{sec:frontend}

As funções do \textit{content script} que injetam o \textit{frontend} na página do Instagram são \texttt{showAnalysing()} (\autoref{lst:showAnalysing}), \texttt{removeAnalysing()}, \texttt{showCensored()}, \texttt{showChecked()} e \texttt{showError()}.
...
\begin{lstlisting}[
  ...
  caption={Função que insere o \textit{frontend} de carregamento na postagem}
]
...
As classes CSS utilizadas para estilizar o \textit{frontend} estão definidas no arquivo \texttt{styles.css} (\autoref{cap:apendice_css}).
...
O script lê \(N\) vezes todas as imagens de uma pasta específica, faz o mesmo redimensionamento e otimização de imagem que o \textit{content script} faz,
...
try:
    ...
    acertou = 'VERDADEIRO' if contem_pessoa == gabarito else 'FALSO'
